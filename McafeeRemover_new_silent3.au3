...
#AutoIt3Wrapper_Res_Comment=1.3.0.0
#AutoIt3Wrapper_Res_Description=Removes McAfee Security Scan & TrueKey 1.3.0.0
...
If FileExists("C:\Program Files\TrueKey") Then
	...
	If ProcessExists('InstallerWrapperService.exe') Then
		...
		$v = $v + 1
		;If FileExists("C:\Program Files\TrueKey") Then
			;$cmd2 = "taskkill.exe /im mcafee* /f /t"
			;RunWait('"' & @ComSpec & '" /c ' & $cmd2, @SystemDir, @SW_HIDE)
		;EndIf
	EndIf
...
If $x = 1 Then
	If FileExists("C:\ProgramData\McAfee\MCLOGS") Then
		...
		$sSlog[$v] = _Now() & " C:\ProgramData\McAfee removed"
		$v = $v + 1
	EndIf
...
Func About()
	...
	MsgBox(0, "", "McAfee Security Scan Plus / True Key Remover" & @CRLF & @CRLF & _
			"Version: 1.3.0.0" & @CRLF & _
			"Your Administrator is removing McAfee Security Scan Plus and/or McAfee True Key" & @CRLF & "CTRL+ALT+m to kill", 5) ; Find the folder of a full path.
EndFunc   ;==>About
